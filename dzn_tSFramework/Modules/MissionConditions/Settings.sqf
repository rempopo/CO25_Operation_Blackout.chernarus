...
MissionCondition1 = [ 
	"WIN"
	, {
		[PolanaArea, "civilian", "(_x in Hostages)", ">= { alive _x } count Hostages"] call dzn_fnc_ccUnits
		&& [ GorkaArea, "west", "", "< 3"] call dzn_fnc_ccUnits
	}
	, "Заложники спасены, террористы уничтожены."
];
MissionCondition2 = ["FAIL", { (Hostages findIf { alive _x }) < 0 }, "Заложники погибли."];
MissionCondition3 = [ "WIPED", { call fnc_isAllDead  }, "All dead", 30 ];
MissionCondition4 = [ "WIN2", { false }, "Заложники спасены, однако части террористов удалось сбежать.", 30 ];